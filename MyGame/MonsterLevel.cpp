...
void MonsterLevel::Update()
{
    ...
    if (spawnedEnemies < MAX_ENEMIES)
    {
        ...
        {
            std::string imagePath = "Assets/monster lvl/monster (" + std::to_string(spawnedEnemies + 1) + ").png";
            QUAD_LOG("Spawning monster enemy: " << imagePath);
            enemies.push_back(std::make_unique<Enemy>(imagePath, rand() % 800, rand() % 600, 4, Enemy::MovementType::ERRATIC));
            spawnTimer = 0.0f;
            spawnedEnemies++;
        }
    }
...
void MonsterLevel::HandleClick(const Quad::Cursor &cursor, bool canFire, int damage)
{
    if (!canFire)
        return;
